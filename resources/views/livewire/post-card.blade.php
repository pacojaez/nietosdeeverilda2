<x-slot name="header">
  <h2 class="font-semibold text-xl text-gray-800 leading-tight">
      {{ __('Espacio para comentar y compartir') }}
  </h2>
</x-slot>

<div class="pb-2 m-4">
  <div class="w-full flex pb-2 m-4 justify-center">
    <div class="bg-gray-100 p-4 justify-center">
        <div class="bg-white border rounded-sm max-w-max lg:max-w-max">
            <div class="flex items-center px-4 py-3">
              {{-- <img class="h-8 w-8 rounded-full" src="https://picsum.photos/id/1027/150/150"/> --}}
              <div class="ml-3">
                <h3><span class="text-2xl font-semibold antialiased block leading-tight">{{ $post->title }}</span></h3>
                <h4><span class="text-gray-600 text-xl block">En {{ $post->place }}</span></h4>
                <h4><span class="text-gray-600 text-xl block">{{ $post->date }}</span></h4>
              </div>
            </div>
            <img alt="{{ $post->title }}" class="block h-auto w-full m-4 p-4" src="{{asset ('storage/images/posts/'.$post->file_name)}}">
            <div class="ml-3">
              <h3><span class="text-xl font-semibold antialiased block leading-tight">{{ $post->body }}</span></h3>
              <h4><span class="text-gray-600 text-lg block">{{ $post->description }}</span></h4>
            </div>
